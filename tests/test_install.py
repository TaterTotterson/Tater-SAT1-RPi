...
class InstallerTests(unittest.TestCase):
    def test_dry_run_uses_latest_tater_and_pinned_satellite_profile(self) -> None:
        ...
        self.assertIn("checkout --detach 8c4e94469856c9b294c21c05373e55dde7a0917a", output)
        self.assertIn("4069417f495d9b5cf4dc9d0a38ce2fbb42d575ae", output)
        self.assertIn("TATER_SETUP_INSTALL_SYSTEM_DEPS=1", output)
        ...
        self.assertIn("tater-sat1-airplay-permissions", output)
        permissions = (ROOT / "script" / "airplay-permissions").read_text()
        self.assertIn("setcap cap_net_bind_service=+ep", permissions)
        self.assertIn("getcap", permissions)
        self.assertIn("tater-sat1-airplay-permissions", (ROOT / "script" / "firstboot").read_text())
    ...
    def test_satellite_dry_run_omits_local_tater(self) -> None:
        ...
        self.assertNotIn("tater-sat1-airplay-permissions", output)
