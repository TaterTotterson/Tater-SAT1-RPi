...
The current Tater CPU installation is intentionally broad. It installs local
model stacks that are inappropriate for a 512 MB Raspberry Pi Zero 2 W.
...
Tater currently imports several feature modules from its main entry point, so
dependency trimming must be paired with import-boundary tests. Simply deleting
packages from `requirements.txt` is not sufficient.
